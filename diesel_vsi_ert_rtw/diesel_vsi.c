...
void diesel_vsi_step(void)
{
...

  {
    ...
    if ((diesel_vsi_DW.TimeStampA >= diesel_vsi_M->Timing.t[0]) &&
      ...
      diesel_vsi_Y.i2 = (diesel_vsi_U.Vload - *lastU) / (diesel_vsi_M->Timing.t
        [0] - lastTime);
    }
    ...
    /* Sin: '<Root>/Sine Wave1' */
    diesel_vsi_B.refvoltage = sin(376.99111843077515 * diesel_vsi_M->Timing.t[0])
      * 170.0;
     ...
     *  Sum: '<S1>/Sum9'
     */
    diesel_vsi_B.Sum12 = 2.0 * rtb_VSIlineZ1 * rtb_MotordeCombustin1 -
      diesel_vsi_U.Vload;
    ...
    diesel_vsi_B.OutputDCVoltage = 0.03 * diesel_vsi_X.Dinmicaeneleje1_CSTATE *
      0.5 * 12000.0 * 0.05 - (diesel_vsi_Y.i2 * diesel_vsi_U.Vload * 1.1 /
      (rtb_MotordeCombustin1 + 1.0) + 0.05 * rtb_MotordeCombustin1);

    /* Sum: '<Root>/Sum9' incorporates:
     *  Inport: '<Root>/Vref'
     */
    rtb_MotordeCombustin1 = diesel_vsi_U.Vref - rtb_MotordeCombustin1;
