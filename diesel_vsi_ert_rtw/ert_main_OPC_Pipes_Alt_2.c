// compile with: "gcc ert_main_OPC_Pipes_Alt_2.c diesel_vsi.c rt_nonfinite.c rtGetInf.c rtGetNaN.c libmcp3204.c  -lm -lwiringPi -lrt -Wall"
/*
 * File: ert_main.c
 *
...
#define OUR_INPUT_FIFO_NAME "/tmp/dataInD"
#define OUR_OUTPUT_FIFO_NAME "/tmp/dataOutD"
...
void rt_OneStep(void)
{
    ...
    rx_length = read(our_input_fifo_filestream, (void*)bufferPipe, 255);		//Filestream, buffer to store in, number of bytes to read (max)
    if (rx_length < 0)
    {
    ...
    {
        //No data waiting
        printf("FIFO Read no bytes\n");
    }
    else
    {
        //Bytes received
        bufferPipe[rx_length] = '\0';
        printf("FIFO %i bytes read : %s\n", rx_length, bufferPipe);
        Vload = strtof(bufferPipe,&pch);
    }
    //======================================================
...
int_T main(int_T argc, const char *argv[])
{
	...
	resultO = mkfifo(OUR_OUTPUT_FIFO_NAME, 0777);		//(This will fail if the fifo already exists in the system from the app previously running, this is fine)
	if (resultO == 0)
	{
		//FIFO CREATED
	...
	resultI = mkfifo(OUR_INPUT_FIFO_NAME, 0777);		//(This will fail if the fifo already exists in the system from the app previously running, this is fine)
	if (resultI == 0)
	{
		//FIFO CREATED
